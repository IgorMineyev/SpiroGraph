...
import { DEFAULT_CONFIG, PRESET_COLORS, BACKGROUND_TEXTURE } from './constants';
...
const App: React.FC = () => {
  ...
  // FIXED: No longer using the background pattern in the main workspace. Pattern is now exclusively in Controls.tsx.
  const backgroundStyle = isScreensaver ? { backgroundColor: '#000000' } : {
  ...
  return (
    ...
    >
      ...
      {!isScreensaver && (
        <div className={`md:hidden p-4 flex justify-between items-center z-30 shrink-0 ${theme === 'dark' ? 'bg-slate-900/40 backdrop-blur-md' : 'bg-white/40 border-b border-slate-200 backdrop-blur-md'}`}>
            ...
            <div className="flex items-center gap-1">
                <button onClick={() => setDownloadState({ active: true, withStats: false })} className={`p-2 rounded-lg transition-colors ${headerActionBtnClass}`} title="Download pattern"><Download size={20} /></button>
                <button onClick={handleRandomize} className={`p-2 rounded-lg transition-colors ${headerActionBtnClass}`} title="Randomize pattern"><Sparkles size={20} /></button>
                <button onClick={startScreensaver} className={`p-2 rounded-lg transition-colors ${headerActionBtnClass}`} title="Start screensaver"><Monitor size={20} /></button>
                ...
                <button onClick={() => setShowControls(true)} className={`p-2 rounded-lg transition-colors ${settingsBtnClass}`} aria-label="Open settings"><Settings size={20} /></button>
            </div>
        </div>
      )}
                  ...
                  {isScreensaver && (
                    <>
                        ...
                        <div className="flex items-center gap-4 md:gap-6 md:ml-4 bg-slate-900 p-3 rounded-xl shadow-lg border border-slate-800/50">
                            <div className="flex justify-between text-[10px] text-slate-500 font-medium px-1"><span>Speed</span><span className="text-slate-500">{config.speed.toFixed(1)}</span></div>
                            <input type="range" min={0.1} max={15} step={0.1} value={config.speed} onChange={(e) => setConfig(c => ({...c, speed: parseFloat(e.target.value)}))} className="h-1 bg-slate-950 rounded-full appearance-none cursor-pointer focus:outline-none transition-colors [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-slate-600 hover:[&::-webkit-slider-thumb]:bg-slate-500" />
                        </div>
                            <div className="flex flex-col gap-1 w-20 md:w-28">
                                <div className="flex justify-between text-[10px] text-slate-500 font-medium px-1"><span>Thickness</span><span className="text-slate-500">{config.lineWidth.toFixed(2)}</span></div>
                                <input type="range" min={0.1} max={15} step={0.1} value={config.lineWidth} onChange={(e) => setConfig(c => ({...c, lineWidth: parseFloat(e.target.value)}))} className="h-1 bg-slate-950 rounded-full appearance-none cursor-pointer focus:outline-none transition-colors [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-slate-600 hover:[&::-webkit-slider-thumb]:bg-slate-500" />
                            </div>
                            ...
                            {!isInfinityMode && (
                                <div className={`flex items-center gap-2 px-2 md:px-3 py-2 rounded-lg font-mono text-[10px] md:text-xs shadow-lg transition-colors select-none bg-slate-900 text-slate-500 border border-slate-800/50`}>
                                    ...
                                    <div className="flex items-center gap-1">
                                        ...
                                        <input type="number" min="0.1" step="0.1" value={durationInput} onChange={handleDurationInputChange} className="themed-spin w-8 md:w-10 bg-transparent text-center border-b border-slate-700 text-slate-500 focus:outline-none focus:border-slate-500 hover:border-slate-600 transition-colors" />
